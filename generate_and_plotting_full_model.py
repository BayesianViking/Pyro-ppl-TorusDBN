...
def plot_data_provide_clusters(mask, Dihedral_train, DSSP_train, AA_train, nr_clusters, hidden_dims, colorbar = True):
    ...
    phipsi_list = torch.empty((int(mask.sum().item()),2))
    
    
    total_previous_seq_len = 0
    
    for i in range(len(Dihedral_train)):
        
        current_seq_len = int(mask[i].item())
        
        phipsi_list[total_previous_seq_len: current_seq_len+total_previous_seq_len] = Dihedral_train[i,:current_seq_len,:]
        
        total_previous_seq_len += current_seq_len
        
    phipsi_done = phipsi_list.numpy()
    kmeans = KMeans(n_clusters=nr_clusters, random_state=0).fit(phipsi_done)
    #kmeans.labels_
    cluster_centers = kmeans.cluster_centers_
    ...
    plt.figure(figsize=(7, 6))
    plt.hist2d( phipsi_done[:,0], phipsi_done[:,1], bins = 200, norm = LogNorm(), cmap = plt.cm.plasma )
